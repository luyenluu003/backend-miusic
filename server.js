...
import { ConversationModel, MessageModel } from "./Schema/ConversationModel.js";
import { getConversation } from "./helps/getConversation.js";
  ...
  try {
    ...
    socket.on('message-page', async (userId) => {
      ...
      const payload = {
        ...
        profile_img: userDetails?.personal_info.profile_img,
    ...
    //messagemodel
    socket.on('messagemodel', async(currentUserId)=>{
...
const generateUploadVideoURL = async () => {
  const date = new Date();
  const videoName = `${nanoid()}-${date.getTime()}.mp4`; 
...
const formatDatatoSend = (user) => {
  const access_token = jwt.sign({ id: user._id, admin: user.admin }, process.env.SECRET_ACCESS_KEY, { expiresIn: "36000m" });
...
server.get("/get-upload-video-url", (req, res) => {
...
//     const link = await upLoadAudio(filenconst user = await User.findOne({ "personal_info.email": email });ame, bucketname, file);
...
server.post("/search-users-friend", (req, res) => {
  let { query } = req.body
  User.find({ "personal_info.username": new RegExp(query, 'i') })
    .limit(50)
    .select()
